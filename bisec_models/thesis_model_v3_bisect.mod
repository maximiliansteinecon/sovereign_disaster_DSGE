...
% (22) SURGICAL BISECTION TEST: replaced with RS pinned at steady state.
% This is the ONLY equation in the entire model where the banking block
% has a LEAD term (RK(+1)) -- removing it drops the forward-looking
% variable count from 7 to 6 (v,c,L,pi,X1,X2 remain; RK no longer does).
% Everything else in the model -- eqs 20,21,23-31 -- is UNCHANGED, so
% this tests exactly one thing: is THIS specific lead-dependency the
% source of the BK failure, with nothing else disturbed.
RS = STEADY_STATE(RS);
