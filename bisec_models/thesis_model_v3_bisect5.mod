...
%     the single ratio (v(+1)/140.573571)^(-chi), an exact identity
...
CEss = 1;                          % rescaled: (vss/140.573571)^(1-chi)=1 identically at SS
...
% ---- ADDED DURING COMMIT 4 ----------------------------------------------
 ...
 % (4a) certainty equivalent, held as its own variable so that
   ...
   % Rescaled by 140.573571: raw v(+1)^(1-chi) has SS value ~1e20
   ...
   % exact. Dividing by the constant 140.573571 first makes CE_ss=1
   % identically; the 140.573571 factors are reinserted in eqs (2)
   % and (4) below so the algebra is UNCHANGED (verified to machine
   % precision against the un-rescaled steady state).
   CE = (v(+1)/140.573571)^(1-chi);
...
v = (c*(1-L)^varpi)^(1-psi) + betatheta*exp((1-psi)*muz)*140.573571*CE^(1/(1-chi));
...
% NOTE: (v(+1)/140.573571)^(-chi) is written as a single ratio, not
% as v(+1)^(-chi) * 140.573571^chi separately. The two are an EXACT
...
% seen when this term was left unscaled -- multiplying by 140.573571^chi
...
% (9) Tobin's Q                                                        (B.5)
Qtob = 1/(1 - tau*(i/k(-1) - 0.110135/4.403200));
% (10) detrended capital law of motion (Gourio factor cancels)         (B.1)
k = ((1-delta0*u^eta)*k(-1) + (i/k(-1) - tau/2*((i/k(-1) - 0.110135/4.403200)^2))*k(-1))/exp(muz);
...
% (22) CAPITAL DEMAND / external finance premium                       (B.17)
...
% ---- PUBLIC AUTHORITY ---------------------------------------------------
% (32) Taylor rule                                                     (B.31)
r = rhor*r(-1) + (1-rhor)*( phipi*(pi-piss) + phiy*(y-0.603260) + 1.015663 ) + sigr*er;
% ---- MARKET CLEARING ----------------------------------------------------
