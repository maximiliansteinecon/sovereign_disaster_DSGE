...
% ---- ADDED DURING COMMIT 4 ----------------------------------------------
 ...
 % (4a) certainty equivalent, held as its own variable so that
   ...
   % BISECTION v3: v(+1) replaced with STEADY_STATE(v) -- removes the
   % lead from CE's OWN definition. CE remains a live variable (not
   % pinned elsewhere), it just necessarily becomes constant (=1) since
   % this is its only defining equation and v(+1) was its only lead.
   CE = (STEADY_STATE(v)/STEADY_STATE(v))^(1-chi);
...
% BISECTION v3: c(+1),L(+1),v(+1) replaced with their STEADY_STATE()
% values -- removes ALL three leads from the SDF equation. Q remains
% live (solved each period from CE, Dcal, Ecal, all contemporaneous),
% just loses its forward-looking component entirely.
Q = beta0*(STEADY_STATE(c)/c)^(-psi)*((1-STEADY_STATE(L))/(1-L))^(varpi*(1-psi))
    * exp(-gamma*muz)
    * (STEADY_STATE(v)/STEADY_STATE(v))^(-chi)
