...
% BISECTION v4: pi(+1) replaced with STEADY_STATE(pi). r remains live,
% solved residually from Q (still pinned by eq 4/SDF) each period.
Q = STEADY_STATE(pi)/r;
...
% BISECTION v4: X1(+1) and pi(+1) replaced with STEADY_STATE() values.
X1 = y*mc + zeta*Theta*STEADY_STATE(X1)*STEADY_STATE(pi)^upsilon;
...
% BISECTION v4: X2(+1) and pi(+1) replaced with STEADY_STATE() values.
X2 = y + zeta*Theta*STEADY_STATE(X2)*STEADY_STATE(pi)^(upsilon-1);
