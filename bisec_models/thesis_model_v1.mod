...
%     >> dynare thesis_model_v1                              % baseline, phi>0
%     >> dynare thesis_model_v1 -DPHIVAL=0                   % counterfactual
...
%  picked one way):
%   * Document B.23 (bank net worth) currently writes R^S_t, R^d_t
%     (contemporaneous). B.17-B.18 establish R^S_t as the rate SET at t
%     for a NEW loan; B.18 correctly uses R^S_{t-1} for a loan already on
%     the books. B.23 describes the SAME loan from the bank's side and
%     should likely also read R^S_{t-1}, R^d_{t-1} for consistency. This
%     .mod's eq (27) already uses the (-1) timing (matching B.18's
%     convention); the document's B.23 has not yet been corrected to
%     match. Zero effect on the steady state either way; affects the IRFs.
